...
if(!dir.exists(paste0(run_dir, "/mhm_", setup))){
  ...
  for(basin in basins){
    ...
    mhm_nml <- sapply(mhm_nml, function(x) gsub("resolution_hydrology(1) = 4000", 
                                                "resolution_hydrology(1) = 1000", x, fixed = TRUE))
    mhm_nml <- sapply(mhm_nml, function(x) gsub("resolution_routing(1) = 4000", 
                                                "resolution_routing(1) = 4000", x, fixed = TRUE))
    ...
    mpr_nml <- sapply(mpr_nml, function(x) gsub(
      "    coord_from_range_step(1:12) = , , 1, , , , , , , , 4000, 4000", 
      "    coord_from_range_step(1:12) = , , 1, , , , , , , , 1000, 1000", x))
...
ggsave(paste0(main_path, "/analysis_results/validation_results/best_validation_1km_runs.png"),
...
ggplot(plot_data[plot_data$variable != "SPAEF" &
  ...
  labs(x="Experiment", fill="TF type", y="") +
  stat_summary(fun.data = get_box_stats, geom = "text", hjust = 0.5, vjust = 0, size=3)
ggsave(paste0(main_path, "/analysis_results/validation_results/best_validation_1km_runs_NoSpaef.png"),
       width = 18, height = 8, units = "in")


write.csv(plot_data, paste0(main_path, "/analysis_results/validation_results/validation_1km_plot_data.csv"))


# Combine with basin info
# main_path <- "C:/Users/morit/Dropbox/Projekte/GenAI_para_for_HLSMs"
# plot_data <- read.csv(paste0(main_path, "/analysis_results/validation_results/validation_2km_plot_data.csv"))
# 
