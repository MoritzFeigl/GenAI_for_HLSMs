...
#' @param variable_list A list with character vectors with variable names that should be sampled from.
#' @param single_var_to_remove variable name, if this variable is the only one existing a sampled function the function will be removed
#' @param necessary_var_to_be_included variable name, if this variable is the not part of a sampled function the function will be removed
...
#' @param file_name character determining the path in which the samples should be saved. ALl samples are stored as file_name/file_name_batch_[1-n_iter].fst
#' @return No returns
#' @export
#' @importFrom magrittr %>%
#' @examples
#' simple_grammar <- create_grammar(a = "<b><op><c>, <a><op><b>, <a><op><c>",
#'                                  b = "var, numeric",
#'                                  c = "var, numeric, <op><a>",
#'                                  op = "+, -, *")
#' \dontrun{
#' grammar_functions <- grammar_sampler(n = 1000,
...
#' variables <- c("x", "y")
#' numbers <- seq(1, 10, 1)
#' variable_list <- list("var" = variables,
#'                       "numeric" = numbers)
...
#'                                     seed = 130422, file_name = "simple_grammar_samples")
#'
#' }
variable_input <- function(functions,
  ...
  if(class(functions) == "character" & length(functions) == 1){
    ...
    fst::threads_fst(nr_of_threads = 1, reset_after_fork = FALSE)
