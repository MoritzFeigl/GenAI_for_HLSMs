...
.distribution_values_from_fun <- function(string, variable_df){
  ...
  # check for excessive NAs --> return error in that case
  if(sum(is.na(f_evaluated))/length(f_evaluated) > 0.05) class(f_evaluated) <- "try-error"
...
.str_format <- compiler::cmpfun(function(x){
  ...
  if(!is.na(as.numeric(x))){
    x <- round(as.numeric(x)/0.01, 0)*0.01
    #if(x > 3) x <- 3
    x <- x %>% format(nsmall=2)
  }
  return(x)})
