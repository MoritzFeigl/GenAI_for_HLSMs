...
class Trainer(object):
    ...
    def __init__(self, model, optimizer, scheduler, vae_loss, config, train_data, val_data):
        self.model = model  # train.torch.prepare_model(model)
    ...
    def train_and_validate(self):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_data.iter_torch_batches(batch_size=self.batch_size):
            ...
            with torch.no_grad():
                for data in self.val_data.iter_torch_batches(batch_size=val_batch_size):
                    x = data["function"].to(device)
                    x2 = data["quantiles"].to(device, dtype=torch.float)
                    if x.shape[0] < self.batch_size:
                        continue
...
def train_loop_per_worker(config, non_opt_para, train_data, val_data):
    # compute training steps
    samples = train_data.count()
    config["training_steps"] = np.floor(samples / config["batch_size"])
    ...
    # Training
    print(f"Number of GPUs: {len(ray.get_gpu_ids())}\n\n\n")
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model = model.to(device)
    trainer = Trainer(model, optimizer, scheduler, vae_loss, config, train_data, val_data)
...
def train_vae(config, non_opt_para):
    ...
    def train_vae_from_non_opt_para(config):
        train_loop_per_worker(config, non_opt_para, train_data, val_data)
...
def tune_vae(config, non_opt_para):
    ...
    # define trainable with resources
    resource_group = tune.PlacementGroupFactory([{"CPU": 2, "GPU": 1}])
    trainable_with_resources = tune.with_resources(train_vae_from_non_opt_para, resource_group)
    ...
    tune_config = tune.TuneConfig(
        ...
        time_budget_s=60 * 60 * 24 * 2.9)  # Stop all trials after 2.9 days = 60*60*24*2.9 seconds
    ...
    if os.path.exists(non_opt_para["results_dir"] + "/" + non_opt_para["run_name"]):
        new_tuner = False
        try:
            tuner = tune.Tuner.restore(path=non_opt_para["results_dir"] + "/" + non_opt_para["run_name"])
        except:
            new_tuner = True
    else:
        new_tuner = False
    ...
    print(f"Best trial final validation accuracy: {best_result.metrics['Train_Accuracy']:.3f}")
